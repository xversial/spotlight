...
path="$HOME/.spotlight/"
item=$(wget -qO- "https://arc.msn.com/v3/Delivery/Cache?pid=279978&fmt=json&ua=WindowsShellClient&lc=fr,fr-FR&ctry=FR" | jq -r ".batchrsp.items | .[0].item")
if [ -z "$item" ]
then
...
path="$path$(date +"%Y%m%d%H%M")-$slug.jpg"
